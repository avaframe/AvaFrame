"""
    Transforming shapefile input to ASCII
"""
...
def shptoasc(avaDir):
    ...
    search_path = os.path.join(avaDir, 'Ravaflow_Input')
    ...
    if len(shpFile) == 1:
        ...
        for root, dir, files in os.walk(search_path):
            for file in files:
                #if file.startswith("primary") and file.endswith(".shp"):
                if file.startswith("primary") and file.endswith(".shp"):
                    shpFile = os.path.join(root, file)
        ...
        ##new raster
        new_out = os.path.join(avaDir, 'Ravaflow_Input', 'rel_rst.tif')
        ...
        if "thickness" in schema:
            ...
            # Arguements
            input_file_path = os.path.join(avaDir, 'Ravaflow_Input', 'rel_rst.tif')
        ...
        ### .tif to ascii
        input_file_path = os.path.join(avaDir, 'Ravaflow_Input', 'rel_rst.tif')
        output_file_path = os.path.join(avaDir, 'Ravaflow_Input', 'rel_rst.asc')
    ...
    if len(shpFile) == 1:
        ...
        ##new raster
        new_out = os.path.join(avaDir, 'Ravaflow_Input', 'rel2_rst.tif')
        drv_tiff = gdal.GetDriverByName("GTiff") 
        chn_ras_ds = drv_tiff.Create(new_out, ras_ds.RasterXSize, ras_ds.RasterYSize, 1, gdal.GDT_Float32)
        chn_ras_ds.SetGeoTransform(geot)
        init_value = 0
...
###------------------------
    ##Same process for thrid release area
    inputDir_rel = pathlib.Path(avaDir, 'Ravaflow_Input')
    shpFile = list(inputDir_rel.glob('*third*.shp'))
    
    if len(shpFile) == 0:
        print("No third release shapefile provided in Ravaflow_Input")
    ...
    if len(shpFile) == 1:
        ...
        for root, dir, files in os.walk(search_path):
            for file in files:
                if file.startswith("third") and file.endswith(".shp"):
                    shpFile = os.path.join(root, file)
        ...
        ##new raster
        new_out = os.path.join(avaDir, 'Ravaflow_Input', 'rel3_rst.tif')
        drv_tiff = gdal.GetDriverByName("GTiff") 
        chn_ras_ds = drv_tiff.Create(new_out, ras_ds.RasterXSize, ras_ds.RasterYSize, 1, gdal.GDT_Float32)
        chn_ras_ds.SetGeoTransform(geot)
        init_value = 0
        ...
        if "thickness" in schema:
            ...
            # Arguements
            input_file_path = os.path.join(avaDir, 'Ravaflow_Input', 'rel3_rst.tif')
        ...
        ### .tif to ascii
        input_file_path = os.path.join(avaDir, 'Ravaflow_Input', 'rel3_rst.tif')
        output_file_path = os.path.join(avaDir, 'Ravaflow_Input', 'rel3_rst.asc')
    ...
    if len(shpFile) == 1:
        ...
        ##new raster
        new_out = os.path.join(avaDir, 'Ravaflow_Input', 'ent_rst.tif')
        drv_tiff = gdal.GetDriverByName("GTiff") 
        chn_ras_ds = drv_tiff.Create(new_out, ras_ds.RasterXSize, ras_ds.RasterYSize, 1, gdal.GDT_Float32)
        chn_ras_ds.SetGeoTransform(geot)
        init_value = 0
...
def cleandirec(avaDir):
    """
    Cleans up the Ravaflow_Input directory and deletes all unnecessary ".tif"-files
    ...
    inputDir = pathlib.Path(avaDir, 'Ravaflow_Input')        

...
* configuration files for all simulations (*Outputs/com1DFA/configurationFiles*)
    ...
    - one file for each simulation that has actually been performed by the latest call of ``runCom1DFA.py`` is saved in (*Outputs/com1DFA/configurationFiles/latestConfigurationFiles*)

    .. Note::
        This kind of storage of configurations from actually performed simulations allows a run that has been terminated
        to be resumed without re-running simulations that have already been performed.

...
velocity) and a threshold of this variable and then compared towards the reference point, line or polygon.
...
----------------------------------------------
In order to include a comparison to reference data sets, the flag ``includeReference`` in your local copy of ``ana3AIMEC/ana3AIMECCfg.ini``
needs to be set to ``True``. After reading the coordinates of the point, line or polygon from the respective shp file, a raster with the same extent
...
Regarding the simulation results, also a runout line is derived. This is done by identifying for each simulation, the
...
in the case of the reference points just a distance between this point and the simulation runout point is computed.

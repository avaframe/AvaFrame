%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Compile with
% pdflatex --shell-escape -synctex=1 -interaction=nonstopmode meshNormal.tex
% to convert it to png use:
% convert -density 300 -transparent white "\image.pdf" "\image.png"},
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\documentclass{standalone}

\input{GeneralPlotHeader.tex}

\begin{document}

\tikzset{
   every node/.style={scale=1.3},
   >=stealth
}

\begin{tikzpicture}[
    scale=3
 ]
\def\xMin{0}
\def\xMinSec{1}
\def\yMin{0}
\def\yMinSec{1}
\def\xMax{2}
\def\yMax{2}

\def\xxMin{-0.5}
\def\xxMinSec{0.5}
\def\yyMin{-0.5}
\def\yyMinSec{0.5}
\def\xxMax{2.5}
\def\yyMax{2.5}
    \foreach \i in {\xMin,\xMinSec,...,\xMax} {
        \draw [gray] (\i,\yMin-0.5) -- (\i,\yMax+0.5);
    }
    \foreach \i in {\yMin,\yMinSec,...,\yMax} {
        \draw [gray] (\xMin-0.5,\i) -- (\xMax+0.5,\i);
        \foreach \j in {\yMin,\yMinSec,...,\yMax} {
            \draw [color=AFMiddle] plot [only marks, mark size=1, mark=square*] coordinates {(\j,\i)};
        }
    }

    \foreach \i in {\xxMin,\xxMinSec,...,\xxMax} {
        \draw [thin, dotted, gray] (\i,\yyMin) -- (\i,\yyMax);
    }
    \foreach \i in {\yyMin,\yyMinSec,...,\yyMax} {
        \draw [thin, dotted, gray] (\xxMin,\i) -- (\xxMax,\i);

    }

\draw (0.75,1.25) node[color=AFDark] {$1$};
\draw (1.25,1.25) node[color=AFDark] {$2$};
\draw (1.25,0.75) node[color=AFDark] {$3$};
\draw (0.75,0.75) node[color=AFDark] {$4$};

\draw[->, very thick, AFDark] (1,1) -- (1,2);
\draw[->, very thick, AFDark] (1,1) -- (2,1);
\draw [dotted, AFDark, very thick] (1,2) -- (2,1);
\draw[->, very thick, AFDark] (1,1) -- (1,0);
\draw [dotted, AFDark, very thick] (2,1) -- (1,0);
\draw[->, very thick, AFDark] (1,1) -- (0,1);
\draw [dotted, AFDark, very thick] (1,0) -- (0,1);
\draw [dotted, AFDark, very thick] (0,1) -- (1,2);


\end{tikzpicture}
\begin{tikzpicture}[
    scale=3
 ]
\def\xMin{0}
\def\xMinSec{1}
\def\yMin{0}
\def\yMinSec{1}
\def\xMax{2}
\def\yMax{2}

\def\xxMin{-0.5}
\def\xxMinSec{0.5}
\def\yyMin{-0.5}
\def\yyMinSec{0.5}
\def\xxMax{2.5}
\def\yyMax{2.5}
    \foreach \i in {\xMin,\xMinSec,...,\xMax} {
        \draw [gray] (\i,\yMin-0.5) -- (\i,\yMax+0.5);
    }
    \foreach \i in {\yMin,\yMinSec,...,\yMax} {
        \draw [gray] (\xMin-0.5,\i) -- (\xMax+0.5,\i);
        \foreach \j in {\yMin,\yMinSec,...,\yMax} {
            \draw [color=AFMiddle] plot [only marks, mark size=1, mark=square*] coordinates {(\j,\i)};
        }
    }

    \foreach \i in {\xxMin,\xxMinSec,...,\xxMax} {
        \draw [thin, dotted, gray] (\i,\yyMin) -- (\i,\yyMax);
    }
    \foreach \i in {\yyMin,\yyMinSec,...,\yyMax} {
        \draw [thin, dotted, gray] (\xxMin,\i) -- (\xxMax,\i);

    }

\draw (0.25,1.25) node[color=AFDark] {$1$};
\draw (0.75,1.75) node[color=AFDark] {$2$};
\draw (1.25,1.75) node[color=AFDark] {$3$};
\draw (1.75,1.25) node[color=AFDark] {$4$};
\draw (1.75,0.75) node[color=AFDark] {$5$};
\draw (1.25,0.25) node[color=AFDark] {$6$};
\draw (0.75,0.25) node[color=AFDark] {$7$};
\draw (0.25,0.75) node[color=AFDark] {$8$};

\draw[->, very thick, AFDark] (1,1) -- (1,2);
\draw[->, very thick, AFDark] (1,1) -- (2,2);
\draw [dotted, AFDark, very thick] (1,2) -- (2,2);
\draw[->, very thick, AFDark] (1,1) -- (2,1);
\draw [dotted, AFDark, very thick] (2,2) -- (2,1);
\draw[->, very thick, AFDark] (1,1) -- (2,0);
\draw [dotted, AFDark, very thick] (2,1) -- (2,0);
\draw[->, very thick, AFDark] (1,1) -- (1,0);
\draw [dotted, AFDark, very thick] (2,0) -- (1,0);
\draw[->, very thick, AFDark] (1,1) -- (0,0);
\draw [dotted, AFDark, very thick] (1,0) -- (0,0);
\draw[->, thick, AFDark] (1,1) -- (0,1);
\draw [dotted, AFDark, very thick] (0,0) -- (0,1);
\draw[->, very thick, AFDark] (1,1) -- (0,2);
\draw [dotted, AFDark, very thick] (0,1) -- (0,2);
\draw [dotted, AFDark, very thick] (0,2) -- (1,2);


\end{tikzpicture}

\end{document}

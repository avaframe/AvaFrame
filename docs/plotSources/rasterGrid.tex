...
\draw[AFLight] (6,\yMin-0.5) node[below] {\footnotesize $N_{x}\!\!-\!\!1$};
...
%\draw (4,\yMin-0.5) node[below] {\footnotesize $N_{cols}-2$};
\draw[AFDark] (5.5,\yMin) node[below] {\footnotesize $N_{x}\!\!-\!\!2$};
...
%\draw (4,\yMin-0.5) node[below] {\footnotesize $N_{cols}-2$};
\draw[AFLight] (\xMin-0.5,5) node[rotate = 90, above] {\footnotesize $N_{y}\!\!-\!\!1$};



\draw[color=AFDark, thick, fill=AFDark, fill opacity=0.3] (3,3) rectangle ++(1,1);
\draw[->, AFDark] (5.5,6) node[above right] {\begin{tabular}{c} Cell \\ $i_c = n_x + n_y(N_x-1)$ \\ $0 \leq n_x \leq (N_x-2)$ \\ $0 \leq n_y \leq (N_y-2)$ \end{tabular}} -- (3.75,3.75);

...
def com4FlowPyMain(cfgPath, cfgSetup):
    ...
    #paula
    xsi_voellmy = float(cfgSetup["xsi_Voellmy"])
    #ende paula
    # Input Paths
    ...
    #paula
    if xsi_voellmy > 0:
        log.info("xsi_voellmy: {}".format(xsi_voellmy))
    # ende paula
    ...
    for i in range(nTiles[0] + 1):
        for j in range(nTiles[1] + 1):
            optList.append((i, j, alpha, exp, cellsize, nodata, flux_threshold,
                            max_z, tempDir, infraBool, nCPU, xsi_voellmy))
    ...
    #Chris
    travel_length = SPAM.MergeRaster(tempDir, "res_travel_length")
    #ende chris
    #Paula
    ...
    #Chris
    io.output_raster(demPath, resDir / ("travel_length%s" % (output_format)), travel_length)
    #ende chris
    #Paula

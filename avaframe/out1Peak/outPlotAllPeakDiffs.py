...
def computeAreaDiff(refRaster, simRaster, thresholdValueReference, thresholdValueSimulation, dem, cropToArea=None):
    ...
    indicatorDict = {'truePositive': {'array': tp, 'nCells': np.nansum(tp), 'areaSum': np.nansum(tp*dem['areaRaster'])},
                     'falsePositive': {'array': fp, 'nCells': np.nansum(fp), 'areaSum': np.nansum(fp*dem['areaRaster'])},
                     'falseNegative': {'array': fn, 'nCells': np.nansum(fn), 'areaSum': np.nansum(fn*dem['areaRaster'])},}

    return refMask, compRasterMask, indicatorDict


def plotAreaDiff(refRaster, refMask, simRaster, compRasterMask, resType, rasterHeader, thresholdValueSimulation, outDir,
                 indicatorDict, simName, cropFile=None):
    ...
    extentCellCenters, extentCellCorners = pU.createExtentMinMax(refRaster, rasterHeader, originLLCenter=True)
    ...
    ax[0].text(0.01, 0.85, ('false negative:\n %.0f cells, %.2f m2' % (indicatorDict['falseNegative']['nCells'],
                                                                       indicatorDict['falseNegative']['areaSum'])),
               horizontalalignment='left', verticalalignment='center', transform=ax[0].transAxes, color='royalblue', alpha=0.75, zorder=5)
    ax[0].text(0.01, 0.95, ('false positive:\n %.0f cells, %.2f m2'% (indicatorDict['falsePositive']['nCells'],
                                                                       indicatorDict['falsePositive']['areaSum'])),
               horizontalalignment='left', verticalalignment='center', transform=ax[0].transAxes, color='darkred', alpha=0.75, zorder=5)
    ax[0].text(0.01, 0.75, ('true positive:\n %.0f cells, %2.f m2'% (indicatorDict['truePositive']['nCells'],
                                                                       indicatorDict['truePositive']['areaSum'])),
               horizontalalignment='left', verticalalignment='center', transform=ax[0].transAxes, color='darkgreen', alpha=0.75, zorder=5)
    title = str("Affected area based on %s %.2f %s" % (resType, thresholdValueSimulation, pU.cfgPlotUtils['unit' + resType]))
    ...
    cmap1, col1, ticks1, norm1 = pU.makeColorMap(
        pU.colorMaps[resType], np.nanmin(refRaster), np.nanmax(refRaster), continuous=True
    )
    cmap1.set_bad(alpha=0)
    im2 = ax[1].imshow(np.ma.masked_where(simRaster == 0.0, simRaster), cmap=cmap1, extent=extentCellCorners, origin='lower', aspect="equal")

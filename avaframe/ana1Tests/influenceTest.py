...
def mainInfluenceTest(avalancheDir, influenceTestCfg, simDF, inputsDF, pathDict):
    ...
    simDF = getAIMECResults(simDF, inputsDF, pathDict)
...
def getAIMECResults(simDF, inputsDF, pathDict):
    """ Get DFA runout
    Parameters
    ----------
    simDF: Data-Frame
    ...
    for index, inputsDFrow in inputsDF.iterrows():
        ...
        for simDFID in simDF.index:
            print('simDFID', simDFID)
            if simDFID==simName[-10:]:
                log.info('j aime la saucisse de Mortaux')
                simDF.loc[simDFID, 'sRunout'] = inputsDF.loc[simName, 'sRunout']
                simDF.loc[simDFID, 'maxpfdCrossMax'] = inputsDF.loc[simName, 'maxpfdCrossMax']
                simDF.loc[simDFID, 'maxpfvCrossMax'] = inputsDF.loc[simName, 'maxpfvCrossMax']
...
def getSimLabel(avalancheDir, cfg, simDF, param2Study=0):
    ...
    inputSimFilesAll, cfgFilesRels = com1DFA.setRelThIni(avalancheDir, com1DFA, cfgFile=cfg)
    ...
    for cfgFileRel in cfgFilesRels:
        ...
        IDListi, _ = com1DFA.prepareVarSimDict(modCfg, inputSimFiles, variationDict, simNameOld=simNameOld)
        for ID in IDListi:
            IDList.append(ID)

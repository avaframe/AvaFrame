...
def mainEnergyLineTest(cfgMain):
    ...
    runOutAngle = np.rad2deg(np.arctan(np.abs(deltaZ/deltaS)))
    ...
    ax2.plot(avaProfileMass['s'][[0, -1]], Zene[[0, -1]], '-r', label='run-out line %.2f °' % runOutAngle)
    ...
    # add alpha line
    ax2.plot(s_geomL, z_geomL, 'b-', label=r'$\alpha$ line %.2f °' % alpha)

    # add horizontal line at the final mass averaged position
    ax2.axhline(y=avaProfileMass['z'][-1], color='k', linestyle='--', label='horizontal line at z final')
    ...
    # add center of mass velocity points and run-out line
    ax3.plot(avaProfileMass['s'][[0, -1]], Zene[[0, -1]], '-r', label='run-out line %.2f °' % runOutAngle)
    ...
    # add alpha line
    ax3.plot(s_geomL, z_geomL, 'b-', label=r'$\alpha$ line %.2f °' % alpha)

    # add horizontal line at the final mass averaged position
    ax3.axhline(y=avaProfileMass['z'][-1], color='k', linestyle='--')
    ax3.axvline(x=avaProfileMass['s'][-1], color='r', linestyle='--')
    ax3.axvline(x=sIntersection, color='b', linestyle='--')

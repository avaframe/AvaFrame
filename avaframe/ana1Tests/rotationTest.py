...
def mainRotationTest(cfgMain, rotationTestCfgFile):
    ...
    rotationTestCfg, modInfo = cfgUtils.getModuleConfig(com1DFA, fileOverride=rotationTestCfgFile, modInfo=True)
    if runDFAModule:
        # Run dense flow
        # here is an example with com1DFA but another DFA computational module can be used
        # as long as it produces some peak flow results
        dem, _, _, simDF = com1DFA.com1DFAMain(avalancheDir, cfgMain, cfgFile=rotationTestCfgFile)
    else:
        # read simulation dem
        ...
        cfgDF, _ = cfgUtils.readAllConfigurationInfo(avalancheDir)
        dataDF, resTypeList = fU.makeSimFromResDF(avalancheDir, 'com1DFA', inputDir='', simName='')
        simDF = cfgDF.reset_index().merge(dataDF, on='simName').set_index('index')
    ...
    cfgSetup['referenceSimName'] = 'rel0'
    cfgAimec['FLAGS']['flagMass'] = 'False'

    # Setup input from com1DFA
    inputsDF, resTypeList = dfa2Aimec.mainDfa2Aimec(avalancheDir, anaMod, cfgAimec)
    ...
    rasterTransfo, resAnalysisDF, plotDict = ana3AIMEC.mainAIMEC(pathDict, inputsDF, cfgAimec)

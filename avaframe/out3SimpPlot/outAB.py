...
def writeABpostOut(header, rasterdata, Avapath, SplitPoint, saveOutPath, flags):
    ...
    for i in range(len(NameAva)):
        ...
        savename = name + 'AlphaBeta_.pdf'
        save_file = os.path.join(saveOutPath, savename)
        plotResults(header, rasterdata, avapath, CoordSplit, eqPost, save_file, flags)
        if flags['WriteRes']:
            WriteResults(eqPost, saveOutPath, name)
...
def WriteResults(eqOutput, saveOutPath, name):
    ...
    FileName_ext = saveOutPath + '/' + name + 'results_python.txt'

...
def test_com7RegionalMain(tmp_path):
    ...
    allPeakFilesDir, allTimeStepsDir, mergedRastersDir = com7Regional.com7RegionalMain(cfgMain, cfg)

    # Check for expected output files and directories
    assert allPeakFilesDir is not None
    assert allPeakFilesDir.exists()
    assert allTimeStepsDir is not None
    assert allTimeStepsDir.exists()

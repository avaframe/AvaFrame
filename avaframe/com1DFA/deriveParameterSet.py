...
def checkResType(fullCfg, section, key, value):
    ...
    if value != '':
        resType = value.split('|')
        validResTypes = ['ppr', 'pft', 'pfv', 'pta', 'pke', 'FT', 'FV', 'FM', 'Vx', 'Vy', 'Vz', 'P', 'TA', 'particles', 'dmDet']
        message = (
...
def checkExtentAndCellSize(cfg, inputFile, dem, fileType):
    ...
    if diffX0 == 0 and diffY0 == 0 and diffY1 == 0 and diffX1 == 0 and inputField['header']['cellsize'] == demHeader['cellsize']:
        if fileType == 'RELTH':

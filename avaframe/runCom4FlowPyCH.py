...
if __name__ == "__main__":
    ...
    for praThreshold in praThresholds:
        for size in np.arange(2,6):
            for ava in ['dry']:
                praScenPath = f'{wsScenPath}/praTh{praThreshold}'
                avaScenName = f'{praScenPath}/Size{size}/{ava}/'
                
                isPath = os.path.isdir(avaScenName)
                
                if isPath is True:
                    log.info("all good starting to calibration brv!")

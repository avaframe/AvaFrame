...
if avalancheDir == 'data/avaSeilbahn':
    print('Extracting AvaNodes data...')
    dictNodes = NodeTools.produceAvaNodesDict(avalancheDir) 
else:
    dictNodes ={} 
...
# Choose if you want the AvaNodes on the plot
AvaNodes = False 
...
# %% Boxplot of the difference between the average velocity of the three AvaNodes with the mean velocity of the simulation flow
if avalancheDir == 'data/avaSeilbahn':
...
# %% Plotting velocity time envelopes for all the simulations in the outputs file
PlotFunctions.plotVelocityTimeEnvelope(number_ava,dictVelEnvelope,dictNodes,Sim,avaDir,AvaNodes=True,Show=False,modName='com1DFA')


# %% Plotting range time diagram with peak flow velocity and the velocity time envelope 
PlotFunctions.plotRangeTimePeakVelVelTimeEnvelope(Sim,avalancheDir,number_ava,dictVelEnvelope,dictNodes,Save=True,Show=False,AvaNodes=True,modName='com1DFA')


# %% Plotting peak flow quantities and the velocity thalweg diagram 
PlotFunctions.plotPeakVelVelThalwegEnvelope(Sim,avalancheDir,number_ava,dictNodes,rasterTransfo,dictVelAltThalweg,Save=True,AvaNodes=True,modName='com1DFA')
...
# %% Boxplot at the whole avalanche scale in the thalweg domain
if avalancheDir == 'data/avaSeilbahn':
    PlotFunctions.plotBoxplotThalweg(avalancheDir,number_ava,Sim,dictNodes,dictVelAltThalweg,TrackedPart=True,Save=True,modName='com1DFA')
...
# %% Boxplot for the tracked particles in the thalweg domain
if avalancheDir == 'data/avaSeilbahn':
    PlotFunctions.plotBoxplotThalweg(avalancheDir,number_ava,Sim,dictNodes,dictVelAltThalwegPart,TrackedPart=True,Save=True,modName='com1DFA')
PlotFunctions.plotVelocityAltitudeThalweg(simu_number,dictVelAltThalwegPart,dictNodes,avaDict,avaDir,rasterTransfo,dictRaster,Title=False,Save=True,AvaNodes=True,TrackedPart=True,Raster=False,EnergyLine=False,modName='com1DFA')


# %% Plotting velocity-altitude-thalweg envelope of the whole avalanche flow for all the different friction parameters 
PlotFunctions.plotVelocityAltitudeThalwegAllSim(dictVelAltThalweg,Sim,dictNodes,avaDir,number_ava,TrackedPart=False,Title=False,Save=True,Show=False,AvaNodes=True,modName='com1DFA')
...
# %% Plotting velocity-altitude-thalweg envelope of the tracked numerical particles for all the different friction parameters 
PlotFunctions.plotVelocityAltitudeThalwegAllSim(dictVelAltThalwegPart,Sim,dictNodes,avaDir,number_ava,TrackedPart=True,Title=False,Save=True,Show=False,AvaNodes=True,modName='com1DFA')
...
from avaframe.Tools import PlotFunctions
PlotFunctions.plotPeakQuantThalTimeEnergyLine(avalancheDir,number_ava,Sim,plotDict,rasterTransfo,dictRaster,dictNodes,Save=True,Show=False,AvaNodes=AvaNodes,modName='com1DFA')

        
# %% Plotting peak flow quantities with tracked particles and velocities 
PlotFunctions.plotPeakQuantTrackedPartVel(avalancheDir,number_ava,Sim,avaDict,dictVelAltThalweg,dictVelAltThalwegPart,trackedPartProp,trackedPartPropAdapted,dictVelEnvelope,dictNodes,Save=True,AvaNodes=AvaNodes,Show=False,modName='com1DFA')

...
def plotAlongProfile(avaDir, peakFilesDF, resType='pft', cInd='names'):
    """ create a four panel plot with pft values along a chosen profile and also on top of DEM
        ...
        inputDir: pathlib path, str
            path to directory where peak files are found - optional
        resType: str
    ...
    for index, row in pftPeakDF.iterrows():
        ...
        depthDict, thicknessRasterNew, slopeAngleField = tF.convertThicknessToDepth(peakRaster, demDict)
    ...
    ax[0, 1].plot(profileLine['s'], demAlitudePoints['z'], color='k', linestyle='-', linewidth=1.4, zorder=11, label='dem elevation\nalong given profile')
...
def fetchCategoricalColors(varParStrValues='', style='line'):
    ...
    cmap34.set_array([])
    cmap35 = mpl.cm.ScalarMappable(norm=norm3, cmap=cmapCrameri.buda)
    cmap35.set_array([])

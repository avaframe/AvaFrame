...
def showSaveTimeSteps(cfgMain, cfg, particlesList, fieldsList, solSimi, Tsave, relDict, outDirTest, simName):
    ...
    while isinstance(value, float):
        ...
        comSol = simiSolTest.prepareParticlesFieldscom1DFA(fields, particles, relDict, simiDict, 'xaxis')
        comSol['outDirTest'] = outDirTest
        comSol['showPlot'] = cfgMain['FLAGS'].getboolean('showPlot')
        comSol['Tsave'] = Tsave[ind_t]

        # make plot
        plotProfilesSimiSol(ind_time, simName, relDict, comSol, simiDict, solSimi, 'xaxis')

        # get particle parameters
        comSol = simiSolTest.prepareParticlesFieldscom1DFA(fields, particles, relDict, simiDict, 'yaxis')
        comSol['outDirTest'] = outDirTest
        comSol['showPlot'] = cfgMain['FLAGS'].getboolean('showPlot')
        comSol['Tsave'] = Tsave[ind_t]

        # make plot
        plotProfilesSimiSol(ind_time, simName, relDict, comSol, simiDict, solSimi, 'yaxis')

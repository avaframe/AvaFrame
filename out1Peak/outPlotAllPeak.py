...
def plotAllPeakFields_ravaflow(avaDir, cfgFLAGS, modName, demData=''):
    """ Plot all peak fields and return dictionary with paths to plots
        with DEM in background
    ...
    # Load all infos on simulations
    avaDir = pathlib.Path(avaDir)
    #print("This is avaDir:",avaDir)
    inputDir = avaDir / 'Outputs' / 'com1DFA' / 'peakFiles'
    #print("This is inputDir:", inputDir)
    peakFilesDF = fU.makeSimDF(inputDir, avaDir=avaDir)

    input_file_path_asc = os.path.join(avaDir, 'Ravaflow_Input', 'DEM.asc')

    if os.path.exists(input_file_path_asc):
        demFile = input_file_path_asc
        demData = IOf.readRaster(demFile, noDataToNan=False)
        demDataField = demData['rasterData']
        demField = demDataField
    ...
    # Loop through peakFiles and generate plot
    #for m in range(len(peakFilesDF['names'])):
    inputDir = avaDir / 'Outputs' / 'r_avaflow_compare' / 'peakFiles'
    peakFiles = list(inputDir.glob('*.asc'))
    
    peakFiles.sort(key=lambda x: str(x).split("_")[-1])
    
    for m,file in enumerate(peakFiles):
        # Load names and paths of peakFiles
        fileName = peakFilesDF["files"][m]
        resType = peakFilesDF['resType'][m]
        log.debug('now plot %s:' % (fileName))
        ...
        plotName = outDir / ('%s.%s' % (file_name, pU.outputFormat))
        ...
        #if not plotName.is_file():
        # Load data
        raster = IOf.readRaster(file, noDataToNan=False)
        data = raster['rasterData']
